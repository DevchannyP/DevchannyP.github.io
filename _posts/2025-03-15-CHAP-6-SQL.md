...
| 유형 | 설명 | 예제 |
| --- | --- | --- |
| **스칼라 서브쿼리** | 하나의 값을 반환하는 서브쿼리 | `SELECT name, (SELECT MAX(salary) FROM employees) AS max_salary FROM employees;` |
| **행 서브쿼리** | 하나의 행(여러 컬럼)을 반환하는 서브쿼리 | `SELECT * FROM employees WHERE (department_id, salary) = (SELECT department_id, MAX(salary) FROM employees GROUP BY department_id);` |
| **테이블 서브쿼리** | 여러 행과 컬럼을 반환하는 서브쿼리 | `SELECT name FROM employees WHERE department_id IN (SELECT id FROM departments WHERE location = 'Seoul');` |
